				...
				{#each items as item, i (item.slug)}
					...
					>
						...
						<div class="flex flex-1 flex-col gap-3 p-7">
							...
							<h2 class="font-serif text-2xl text-ink italic">{item.name}</h2>
							<p class="text-sm leading-relaxed text-ink-soft">{item.desc}</p>
							{#if item.layanan}
								<div class="mt-1 flex flex-wrap gap-2">
									{#each item.layanan as layanan (layanan)}
										<span
											class="rounded-full border border-ink/15 px-3 py-1 font-mono text-[10px] tracking-[0.05em] text-ink-soft uppercase transition-colors hover:border-clay hover:text-clay"
										>
											{layanan}
										</span>
									{/each}
