...
<svelte:head>
	<title>{item ? `${item.name} - Potensi Desa Cipicung` : 'Potensi Desa - Desa Cipicung'}</title>
</svelte:head>
		...
		<div class="mx-auto max-w-4xl">
			...
			>
				<div use:reveal class="reveal-scale w-full shrink-0 md:w-64">
					<div class="aspect-square w-full overflow-hidden rounded-sm">
						<img src={item.gambar} alt={item.name} class="h-full w-full object-cover" />
					</div>
				</div>
				...
				<div use:reveal={{ delay: 140 }} class="reveal-up flex-1">
					...
					<h1 class="mt-2 font-serif text-4xl italic md:text-5xl">{item.name}</h1>
					...
					{#if item.layanan}
						<div class="mt-8 border-t border-ink/15 pt-6">
							...
							<div class="mt-3 flex flex-wrap gap-2">
								{#each item.layanan as layanan, i (layanan)}
									<span
										use:reveal={{ delay: 200 + i * 70 }}
										class="reveal-up cursor-default rounded-full border border-ink/20 px-4 py-1.5 font-mono text-xs text-ink transition-all hover:border-clay hover:text-clay active:scale-95"
									>
										{layanan}
									</span>
								{/each}
